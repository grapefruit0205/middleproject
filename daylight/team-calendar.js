...
for (const [id, offset] of [['previous-week', -7], ['next-week', 7]]) document.querySelector(`#${id}`).addEventListener('click', () => {
  const date = new Date(selectedDate); date.setDate(date.getDate() + offset); selectDate(date);
});
...
function submitTeamEvent(event) {
  ...
  document.querySelector('.week-scroll').scrollTop = Math.max(0, (start / 60 - 1) * hourHeight());
...
function renderTeamCalendar() {
  ...
  const range = `${dateKey(dates[0])} ~ ${dateKey(dates[6])}`;
  document.querySelector('#week-range').textContent = range;
  document.querySelector('.week-scroll').setAttribute('aria-label', `${range} 주간 일정`);
  filterEvents();
}
...
function renderTeamNotifications() {
  ...
  for (const notification of items) {
    ...
    item.append(title, meta); item.addEventListener('click', () => {
      ...
      document.querySelector('.week-scroll').scrollTop = Math.max(0, (entry.start / 60 - 1) * hourHeight());
    }); list.append(item);
  }
}
...
document.querySelector('.week-scroll').scrollTop = 8 * hourHeight();
