...
Phase 12~18은 `tools/orchestration/phases-12-plus.json`을 사용한다. CMDC 구현 모델은 모든 Phase에서 `deepseek/deepseek-v4-flash`를 사용한다. effort는 `max`, 최초·수정 시도는 각각 100턴이며 Phase당 유효 호출은 최대 2회다. runner가 프로세스 결과를 받은 호출만 횟수에 포함하고, 결과 없이 중단된 실행은 활성 writer와 작업 트리 변경이 없음을 확인한 뒤 미차감 복구한다. Codex 메인이 각 Phase를 `gpt-5.6-sol` / `high`로 독립 검증하며 상태는 `.orchestration/phase-12-18-state.json`에 분리한다.
