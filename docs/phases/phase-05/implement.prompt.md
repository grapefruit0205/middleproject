...
Architecture v1.2, ADR-001, ADR-002, Phase 05 brief를 읽어라. Apache는 React 정적 파일과 `/api/*` Reverse Proxy만 담당한다. Proxy 대상은 Internal ALB DNS다. WAS는 Java 21과 외장 Tomcat 10.1에서 `ROOT.war`를 실행한다.
