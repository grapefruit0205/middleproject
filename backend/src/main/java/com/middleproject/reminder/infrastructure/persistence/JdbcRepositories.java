...
class JdbcReminderRepository implements ReminderRepository {
    ...
    public Reminder insert(UUID id, UUID eventId, UUID policyId) {
        OffsetDateTime now = OffsetDateTime.now();
        db.update("insert into reminders(id,event_id,policy_id,status,created_at,updated_at,version) values(?,?,?,?,?,?,0)", id, eventId, policyId, ReminderStatus.CREATED.name(), now, now);
        return findById(id).orElseThrow();
    }
